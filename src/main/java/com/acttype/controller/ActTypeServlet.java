	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/acttype/select_page.jsp");
				...
				ActTypeService actTypeSvc = new ActTypeService(); //New a facade(ActTypeService) to call ActTypeDAO methods.
				ActTypeVO actTypeVO = actTypeSvc.getOneById(actCategoryId);  //Call ActTypeDAO method "getOneById(actCategoryId)" and save result as actTypeVO.
				if(actTypeVO == null) {   
					errorMsgs.add("查無資料");
				}
				if (!errorMsgs.isEmpty()) {
				...
				String url = "/front-end/acttype/listOneActType.jsp";
		...
		if ("update".equals(action)) { // 來自update_acttype_input.jsp的請求 <input type="hidden" name="action" value="update">
			...
			try {
				...
				String actCategoryNameReg = "^[(\u4e00-\u9fa5)(a-zA-Z0-9_)]{2,11}$"; //(中文unicode)(英文大小寫)(數字0-9) ;[any reg matched];
				...
				//先去流程圖 check 評價流程
				Integer actNumber = null;
				try {
					actNumber =new Integer (req.getParameter("actNumber").trim());
				} catch (NumberFormatException e) {
					actNumber = 0;
				...
				//先去流程圖 check 評價流程
				Integer actTotalScore = null;
				try {
					actTotalScore =new Integer (req.getParameter("actTotalScore").trim());
				} catch (NumberFormatException e) {
					actTotalScore = 0;
		...
		req.setAttribute("actTypeVO", actTypeVO);  //update出錯 原因在這裏沒set到VO,所以JSP沒辦法成功驗證input data 的format.
				...
				ActTypeService actTypeService  = new ActTypeService();
				actTypeVO = actTypeService.updateActType(actCategoryId, actCategoryName, actCategoryDesc, actMaxPart, actMinPart, actFee, actNumber, actTotalScore);
        ...
        if ("insert".equals(action)) { // 來自addActType.jsp的請求  
			...
			try {
				...
				ActTypeService actTypeService  = new ActTypeService();
				actTypeVO = actTypeService.addActType(actCategoryName, actCategoryDesc, actMaxPart, actMinPart, actFee, actNumber, actTotalScore);
		...
		if ("delete".equals(action)) { // 來自listAllActType.jsp
			...
			try {
				...
				ActTypeService actTypeService  = new ActTypeService();
				actTypeService.deleteActType(actCategoryId);
