...
@WebServlet("/room/roomDoneServlet")
public class RoomDoneServlet extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        oneRm.setRoomInformation(null);//清空住客資訊
        oneRm.setRoomCheckStatus((byte) 0);//修改未使用
        oneRm.setRoomSaleStatus((byte) 0);//修改可用
        RmVO rmVO = rmService.updateRmVO(oneRm);

        //移除詳細列表

        extraBillService.deleteExtraBill(rmVO.getRoomId());
