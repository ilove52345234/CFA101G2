...
<table id="table-1">
   <tr><td><h3>G2 ActType: Home</h3><h4>( MVC )</h4></td></tr>
</table>

<p>This is the Home page for G2 ActType: Home</p>
...
<ul>
  <li><a href='<%=request.getContextPath()%>/back-end/acttype/listAllActType.jsp'>List</a> all ActTypes.  <br><br></li>
     ...
     <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/acttype/ActTypeServlet" >
       ...
       <select size="1" name="actCategoryId">
         <c:forEach var="actTypeVO" items="${actTypeSvc.all}" > 
          <option value="${actTypeVO.actCategoryId}">${actTypeVO.actCategoryId}
         </c:forEach>   
       </select>
       <input type="hidden" name="action" value="getOne_For_Display">
       <input type="submit" value="送出">
...
<ul>
  <li><a href='<%=request.getContextPath()%>/back-end/acttype/addActType.jsp'>Add</a> a new ActType.</li>
</ul>
