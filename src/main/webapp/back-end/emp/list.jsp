...
<div class="container" style="margin-top: 0px">
    ...
    <div style="float: right;margin: 5px;">
        ...
        <a class="btn btn-primary" href="<%=request.getContextPath()%>/back-end/emp/add.jsp">新增管理員</a>
    ...
    <form id="empForm" action="<%=request.getContextPath()%>/emp/delSelectedServlet?currentPage=${pb.currentPage}&rows=5" method="post">
        ...
        <table border="1" class="table table-bordered table-hover">
            <tr class="success" >
                <th class="th"><input type="checkbox" id="firstCB"></th>
                <th class="th">列表號碼</th>
                <th class="th">管理員號碼</th>
                <th class="th">姓名</th>
                <th class="th">帳號</th>
                <th class="th">密碼</th>
                <th class="th">管理員狀態</th>
                <th class="th">新增日期</th>
                <th class="th">權限</th>
                <th class="th">操作</th>
            </tr>
...
<%--                    <td>${emps.empPassword}</td>--%>
                    <td id="password${emps.empId}" onclick="encode('${emps.empPassword}','${emps.empId}')" class="pass"></td>
                    <td>
...
<script>
    ...
    function encode(password,id) {

        let s = "password"+id;

        let encode = Base.encode(password);
        let elementById = document.getElementById(s);
        elementById.innerText = encode;
    }


    window.onload = function () {
        $(".pass").trigger("onclick")
        //給刪除選中按鈕添加點擊事件
